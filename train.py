...
def init():
    ...
    # log the config
    logger.info(cfg)
    os.makedirs(os.path.join(cfg.exp_path,"best checkpoint"), exist_ok=True)
    # summary writer
    global writer
...
def eval_epoch(val_loader, model, model_fn, epoch):
    ...
    with torch.no_grad():
        ...
        is_best = am_dict['loss'].avg <best_val
        if is_best:
            best_val = min(am_dict['loss'].avg, best_val)
            utils.save_best_checkpoint(model, optimizer, os.path.join(cfg.exp_path,"best checkpoint") , cfg.config.split('/')[-1][:-5], epoch, use_cuda)
            with open(os.path.join(cfg.exp_path,"best checkpoint","best_val.txt"),"w") as f_n:
                f_n.write(str(best_val))
...
if __name__ == '__main__':
    ...
    if start_epoch != 1:
        try:
            with open(os.path.join(cfg.exp_path,"best checkpoint","best_val.txt"),"r") as f_n:
                best_val= np.float(f_n.read())
        except :
            best_val = 100000
    else :
        best_val = 100000
