...
class ASPP(SparseModule):
    def __init__(self,in_channels, out_channels, norm_fn, indice_key=6, rate=[6, 12, 18,24]):
        ...
        self.aspp_block4 = spconv.SparseSequential(
            spconv.SparseConv3d(
                in_channels, out_channels, 3, stride=1, padding=rate[3], dilation=rate[3], bias=False,  indice_key="bb_ASPPspconv{}".format(indice_key)
            ),
            nn.ReLU(),
            norm_fn(out_channels),
        )
    ...
    def forward(self, input):
        ...
        out.features = torch.cat((x1.features, x2.features,x3.features,out.features), dim=1)
